    ...
    public function bulk_import_data(Request $request)
    {
        ...
        foreach ($collections as $collection) {
            ...
            array_push($data, [
                ...
                'recommended' => $collection['Recommended'] == 'yes' ? 1 : 0,
                'status' => $collection['Status'] == 'active' ? 1 : 0,
				'avg_rating' => $collection['AvgRating'],
				'rating_count' => $collection['TotalRatingCount'],
                'updated_at'=>now()
            ]);
        }
        ...
        try{
            ...
            foreach($chunk_items as $key=> $chunk_item){
                DB::table('food')->upsert($chunk_item,['id'],['name','description','image','category_id','category_ids','price','discount','discount_type','available_time_starts','available_time_ends','variations','add_ons','restaurant_id','status','veg','recommended','avg_rating','rating_count']);
            }
            DB::commit();
        }catch(\Exception $e)
        {
