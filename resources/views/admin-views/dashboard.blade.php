        ...
        <div class="page-header">
            <div class="d-flex flex-wrap justify-content-between align-items-center">
                <div class="page--header-title">
                    ...
                    <p class="page-header-text">{{translate('messages.welcome_message')}}<button style="float:right" type="button" onclick="fuction_fush_tempfile();" class="btn btn--primary">Fuese Temp Files</button></p>
                </div>
...
function fuction_fush_tempfile()
{
	alert("its working Sumon Sir. just matter of time");
	alert(baseUrl);
	var eventid=1;
	$.ajax({
			type:"GET",
			dataType:"json",
				data:{eventid: eventid},
				 url:baseUrl+"sorage/fuese",
                beforeSend: function () {
                    
					alert(baseUrl+"sorage/fuese");
					$('#loading').show()
			  ...
			  success:function(result){
				 ...
				 //alert(result.item_name);
				  if (result.status == 'success'){
					  ...
					  $('#loading').show()
				  	//$(".yet-to-do").append("<li id=\"li-"+result['id']+"\"><label><input onclick=\"changeListItemStatus('"+eventid+"', "+eventid+")\" type=\"checkbox\" id=\"item-"+eventid+"\" /> "+itemid+"</label>&nbsp; &nbsp;<a href=\"#\" class=\"delete-to-do text-danger\" onclick=\"return deleteToDoItem('"+eventid+"', "+eventid+")\"><i class=\"fas fa-minus-circle\"></i></a></li>");
					//$("#new-to-do").val('');
					//$(".not-found-to-do").hide();
				  }
				  else
				  {
				  	alert('You request could not be processed. Please try later.');
				  }
				//$(".to-do-wait").hide(0);
				}
				,
