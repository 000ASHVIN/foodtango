    ...
    <div class="content container-fluid">
        ...
        <div class="row digital_payment_methods mt-3 g-3">
            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-body p-30px">
                        @php($config=\App\CentralLogics\Helpers::get_business_settings('plural'))
                        <form
                            action="{{env('APP_MODE')!='demo'?route('admin.business-settings.payment-method-update',['plural']):'javascript:'}}"
                            method="post">
                            @csrf
                        <h5 class="d-flex flex-wrap justify-content-between">
                            <strong>{{translate('Plural')}}</strong>
                            <label class="switch--custom-label toggle-switch toggle-switch-sm d-inline-flex">
                                <span class="mr-2 switch--custom-label-text text-primary on text-uppercase">{{ translate('messages.on') }}</span>
                                <span class="mr-2 switch--custom-label-text off text-uppercase">{{ translate('messages.off') }}</span>
                                <input type="checkbox" class="toggle-switch-input" name="status" value="1" {{$config?($config['status']==1?'checked':''):''}}>
                        ...
                        <div class="payment--gateway-img">
                            <img src="{{asset('/public/assets/admin/img/payment/plural.svg')}}" alt="public">
                        </div>
                            <div class="form-group mb-4">
                                <label for="mode">{{ translate('messages.select_payment_mode') }}</label>
                                <select name="mode" class="form-control" id="sel1">
                            ...
                            <div class="form-group mb-4">
                                <input class="form-control" type="text" placeholder="Plural MID" name="plural_mid"
                                           value="{{env('APP_MODE')!='demo'?($config?$config['plural_mid']:''):''}}">
                            </div>
                            <div class="form-group mb-4">
                                <input class="form-control" type="text" placeholder="Plural Access Key" name="plural_access_key"
                                           value="{{env('APP_MODE')!='demo'?($config?$config['plural_access_key']:''):''}}">
                            </div>
                            <div class="form-group mb-4">
                                <input class="form-control" type="text" placeholder="Plural Secret" name="plural_secret"
                                           value="{{env('APP_MODE')!='demo'?$config['plural_secret']??'':''}}">
                            </div>
                            <div class="text-right">
                                <button type="{{env('APP_MODE')!='demo'?'submit':'button'}}"
                                        onclick="{{env('APP_MODE')!='demo'?'':'call_demo()'}}" class="btn h--37px btn--primary">{{translate('messages.save')}}</button>
